...
def configure_logging(level: str = "INFO", redaction_patterns: list[str] | None = None) -> None:
    ...
    structlog.configure(
        ...
        # Critical: route structlog output to stderr, NOT stdout.
        # Stdout is reserved for the MCP JSON-RPC protocol; anything written
        # there corrupts the message stream. PrintLoggerFactory defaults to
        # stdout, so we must override it explicitly.
